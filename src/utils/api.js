export async function getWeather(lat, lng) {
  const r = await fetch(
    ...
    `&daily=wind_speed_10m_max,wind_gusts_10m_max,wind_direction_10m_dominant,temperature_2m_max,temperature_2m_min,precipitation_sum,sunrise,sunset` +
    `&wind_speed_unit=kn&timezone=auto&forecast_days=8`
  );
...
export async function getMarine(lat, lng) {
  try {
    const r = await fetch(
      ...
      `&daily=wave_height_max,wave_period_max,swell_wave_height_max,swell_wave_direction_dominant` +
